...
const services=[{name:'Natural manicure',price:'1,800 ₽',duration:'60 minutes'},{name:'Manicure + gel polish',price:'2,800 ₽',duration:'90 minutes'},{name:'French manicure',price:'3,300 ₽',duration:'120 minutes'}];
...
$('bookingForm').addEventListener('submit',e=>{e.preventDefault();if(!$('bookingForm').reportValidity())return;const s=services[+$('serviceSelect').value];const day=new Date($('dateInput').value+'T12:00:00').toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'});const time=new FormData(e.target).get('time');$('confirmService').textContent=s.name+' · '+s.price;$('confirmDate').textContent=day+', '+time+' · '+s.duration;$('confirmQuiet').textContent=$('quiet').checked?'Your preference: a quiet appointment.':'';$('bookingForm').hidden=true;$('confirmation').hidden=false;$('editBooking').focus()});
